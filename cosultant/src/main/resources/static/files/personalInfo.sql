   ...
   ZIP NUMBER(5) not null
    ...
    CONSTRAINT FK_ADDRESS FOREIGN KEY (ADDRESS_ID) REFERENCES ADDRESS (ADDRESS_ID)
        ...
        CREATE TABLE DOCUMENTS (
            ...
            CONSTRAINT FK_DOC_EMP FOREIGN KEY (EMPLOYEE_ID) REFERENCES PERSONAL_INFO(EMPLOYEE_ID)            
        );
                     
             select * from vendor 
             select * from documents
             delete from documents where doc_id is not null
             select * from personal_info where UPPER(first_name) like UPPER('%S%')
             update personal_info set RECRUITER_NAME = 'Anil KC' where employee_id = 11495
              
            delete from EMPLOYEMNT_HIST where EMP_ID is not null
            
DROP TRIGGER LAST_UPDATED_PERSONAL_INFO

CREATE OR REPLACE TRIGGER LAST_UPDATED_PERSONAL_INFO BEFORE INSERT OR UPDATE ON PERSONAL_INFO 
FOR EACH ROW
BEGIN
...
CREATE OR REPLACE TRIGGER LAST_UPDATED_DOCUMENTS BEFORE INSERT OR UPDATE ON DOCUMENTS
FOR EACH ROW
BEGIN
    :NEW.LAST_UPDATED := SYSDATE;
END;
